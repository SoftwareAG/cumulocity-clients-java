...
public class CodecServiceTest extends TestCase {
...
//        Assert.assertEquals(deviceTypeMo.get(C8Y_LPWAN_CODEC_DEVICE_INFO), supportsDevices().stream().findFirst().get().getAttributes());
        Assert.assertEquals(deviceTypeMo.getName(), supportsDevices().stream().findFirst().get().getDeviceTypeName());
...
//        Assert.assertEquals(deviceTypeExtId.getType(), C8Y_SMART_REST_DEVICE_IDENTIFIER);
        Assert.assertEquals(deviceTypeExtId.getExternalId(), supportsDevices().stream().findFirst().get().getDeviceTypeName());
    }
    ...
    private void setUpIsDeviceTypeExists(boolean isDeviceTypeExists) {
        when(codecMicroservice.supportsDevices()).thenReturn(supportsDevices());
...
//        codecService.registerDeviceTypes(microserviceSubscriptionAddedEvent);
    ...
    private Set<DeviceInfo> supportsDevices() {
